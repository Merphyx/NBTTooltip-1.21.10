...
public class NBTTooltip implements ClientModInitializer {
	...
	private static final KeyBinding.Category category = KeyBinding.Category.create(Identifier.of("nbttooltip", "headline"));
		...
		if (ModConfig.INSTANCE.triggerType.shouldShowTooltip(context, type)) {
			...
			if (!tag.isEmpty()) {
				if (ModConfig.INSTANCE.showDelimiters) {
					ttip.add(Text.literal(Formatting.DARK_PURPLE + " - component(s) start -"));
				}
				...
				if (ModConfig.INSTANCE.showDelimiters) {
					ttip.add(Text.literal(Formatting.DARK_PURPLE + " - component(s) end -"));
				}
				ttip = NBTTooltip.transformTtip(ttip, lines);
				list.addAll(ttip);
			} else {
				list.add(Text.literal(FORMAT + "No component(s) data"));
			}
		}
	}
	...
	private static void copyToClipboard(ItemStack stack, MinecraftClient mc) {
		...
		getCopyingEngine().parseTagToList(nbtData, stack, encodeStack(stack, mc.player.getRegistryManager().getOps(NbtOps.INSTANCE)), false);
		nbtData.forEach(t -> {
